...
def structured_case_ids(e):
    """Extract case IDs only from structured interaction targets, not OCR/setup text."""
    p=e.get('payload') or {}; values=[]
    target=p.get('target_element')
    if isinstance(target,dict): values.extend([target.get('name'),target.get('automation_id'),target.get('value')])
    element=p.get('element')
    if isinstance(element,dict):
        values.extend([element.get('name'),element.get('text'),element.get('value'),element.get('id')])
        attrs=element.get('attributes')
        if isinstance(attrs,dict): values.extend(v for v in attrs.values() if isinstance(v,str))
    ids=set()
    for v in values:
...
def run(a_root,b_root,out):
    ...
    for (sid,cid),es in case.items():
        ...
        if ts: seg.append({'session_id':sid,'start_ms':max(min(ts)-4600,0),'end_ms':max(ts)+5000,'label':'workflow_'+cid.split('-')[0].lower(),'anchor':cid})
